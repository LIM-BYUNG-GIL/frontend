// 자바스크립트 데이터타입 : 원시데이터타입(number, string, boolean, undefined, null 등)을 제외한 합수, 배열도 객체 object 입니다. 데이터 타입확인 연산자 typeof
// 객체 구성 : 프로퍼티(객체의 상태), 메소드(객체의 동작/기능) 객체의 프로퍼티 값이 함수일때 메소드 입니다.
// 객체 정의 : 자바에서는 클래스를 먼저 정의하고 필요할때 new 연산으로 생성
//            자바스크립트는 프로토타입 기반(별도 클래스 저으이 없는 형식)이라고 합니다. 바로 객체 리터럴로 생성할 수 있습니다.

let person = { name : 'sana',
               age : 19,
               sayHi : function() {     // 메소드 정의 : 객체의 프로퍼티 값이 함수 정의일때 메소드 입니다.
...
// JSON : 자바스크립트 객체와 서로 변화되는 문자열 (onject <-> JSON)
